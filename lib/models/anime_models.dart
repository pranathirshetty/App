...
class AnimeItem {
  ...
  String get imageUrl => coverImage.bestUrl.isNotEmpty
      ? coverImage.bestUrl
      : (banner?.startsWith('http') == true ? banner! : '');
  String? get bannerUrl =>
      (bannerImage?.isNotEmpty == true) ? bannerImage : (banner?.startsWith('http') == true ? banner : null);
  ...
  factory AnimeItem.fromProjectRJson(Map<String, dynamic> json) {
    ...
    final coverData = json['cover_image'];

    return AnimeItem(
      id: json['anime_id'] ?? json['id'] ?? json['slug'] ?? '',
      animeId: json['anime_id'] ?? json['id'] ?? json['slug'] ?? '',
      anilistId: json['anilist_id'] ?? json['anilistId'],
      malId: json['mal_id'] ?? json['malId'],
      title: titleData is Map<String, dynamic>
          ? AnimeTitle.fromJson(titleData)
          : AnimeTitle(english: titleData?.toString() ?? ''),
      coverImage: coverData is Map<String, dynamic>
          ? CoverImage.fromJson(coverData)
          : CoverImage(extraLarge: json['cover'] ?? json['image'] ?? ''),
...
class AnimeTitle {
  ...
  factory AnimeTitle.fromJson(Map<String, dynamic> json) {
    return AnimeTitle(
      ...
      romaji: json['romaji'] ?? '',
      nativeLanguage: json['native'] ?? '',
      userPreferred: json['user_preferred'] ?? json['userPreferred'] ?? '',
    );
  }

  String get displayTitle =>
      english.isNotEmpty ? english : romaji.isNotEmpty ? romaji : userPreferred.isNotEmpty ? userPreferred : nativeLanguage;
}
